...
def testing(epoch, actor_model,log_file,cdn_selection, total_td_loss):
    ...
    rewards = np.array(rewards)  # test rewards
...
def central_agent(net_params_queues, exp_queues, model_type,best_hyperparams):
    ...
    for epoch in range(TOTALEPOCH):
        ...
        cdn_selections[cdn_selection] += 1
        print('cdn selected: ', cdn_selection)
    ...
    counts = list(cdn_selections.values())
    plt.bar(cdn_names,counts)
    plt.xlabel('CDN')
    plt.ylabel('Count')
    plt.title('CDN Selections over Epochs')
